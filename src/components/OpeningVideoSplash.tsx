...
export function OpeningVideoSplash() {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center bg-primary/95 transition-opacity duration-700 ${
    ...
    >
      <button
        ...
        className="absolute right-4 top-4 z-10 inline-flex items-center gap-1 rounded-full border border-white/30 bg-black/40 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-white backdrop-blur transition-all hover:bg-black/60"
      >
        Skip
      </button>
      <div className="absolute left-4 top-4 z-10 text-[11px] font-semibold uppercase tracking-[0.24em] text-white/80">
        Pacific North Events &amp; Tents
      </div>
      <video
        ref={videoRef}
        src={openingVideo}
        className="h-full w-full object-cover sm:rounded-2xl sm:max-h-[90vh] sm:max-w-[90vw]"
        autoPlay
        playsInline
        preload="auto"
        onEnded={finish}
